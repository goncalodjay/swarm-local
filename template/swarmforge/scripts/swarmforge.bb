          ...
          (if (or (str/blank? line) (str/starts-with? line "#"))
            ...
            (let [fields (str/split line #"\s+")]
              ...
              (let [[keyword role agent worktree & trailing] fields
                ...
                (when-not (#{"claude" "codex" "copilot" "grok" "opencode" "pi"} agent)
  ...
  (let [role (:role row)
    ...
    (cond-> (str base
                (case agent
                  ...
                  "opencode" (str "opencode " (extra-args-prefix row) "--prompt \"$(cat " (sq (str prompt-file)) ")\"")
  ...
  (when-not (= "0" (System/getenv "SWARMFORGE_PREVENT_SLEEP"))
    (case (uname)
      "Darwin" (when (command-exists? "caffeinate")
                 ["caffeinate" "-dims"])
      "Linux" (let [inhibitor ["systemd-inhibit"
                                "--what=sleep:idle"
                                "--who=SwarmForge"
                ...
                (when (and (command-exists? "systemd-inhibit")
                           ...
                           ;; Some containers report systemd as running but do
                           ;; not permit logind inhibitors. Probe before using
                           ;; the wrapper, or the handoff daemon exits before
                           ;; it can process the outbox.
                           (apply sh-ok? (conj inhibitor "true")))
                  inhibitor))
      nil)))
